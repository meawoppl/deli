...
public class TriangleSoup extends AbstractSet<Triangle> implements Set<Triangle> {
  ...
  public TriangleSoup transform(PointTransformer pointTransformer) {
    return new TriangleSoup(
        this.triangles.stream()
            .map(triangle -> triangle.transform(pointTransformer))
